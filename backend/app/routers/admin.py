...
from ..models import Show, Season, Episode, Artwork, ArtworkKind, User
from ..schemas import ShowIn, ShowOut, SeasonIn, SeasonOut, EpisodeIn, EpisodeOut, \
    ArtworkOut, ValidationReport, PublishRunOut
...
ARTWORK_SPECS = {
    ArtworkKind.poster: {"ratio": (2, 3), "target": (600, 900), "tolerance_pct": 5},
    ArtworkKind.banner: {"ratio": (16, 9), "target": (1280, 720), "tolerance_pct": 5},
    ArtworkKind.thumbnail: {"ratio": (16, 9), "target": (640, 360), "tolerance_pct": 5},
}
...
def create_show(payload: ShowIn, db: Session = Depends(get_db), _user: User = Depends(require_editor)):
...
def update_show(show_id: str, payload: ShowIn, db: Session = Depends(get_db), _user: User = Depends(require_editor)):
...
def set_show_publish_flag(show_id: str, published: bool, db: Session = Depends(get_db),
                           _user: User = Depends(require_editor)):
...
def create_episode(season_id: str, payload: EpisodeIn, db: Session = Depends(get_db),
                    _user: User = Depends(require_editor)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(409, f"(content_group='{payload.content_group}', language='{payload.language}') already exists.")
    db.refresh(episode)
    return episode


@router.post("/episodes/{episode_id}/publish-flag", response_model=EpisodeOut)
def set_episode_publish_flag(episode_id: str, published: bool, db: Session = Depends(get_db),
                              _user: User = Depends(require_editor)):
    ...
    if published:
        if not ep.duration_seconds:
            raise HTTPException(422, "Cannot publish an episode with no duration.")
        if not ep.artworks and not ep.season.show.artworks:
            raise HTTPException(422, "Cannot publish an episode with no artwork.")
...
# ---------- Artwork upload ----------
def _validate_dimensions(kind: ArtworkKind, width: int, height: int) -> Optional[str]:
    spec = ARTWORK_SPECS[kind]
    tw, th = spec["target"]
    tol = spec["tolerance_pct"] / 100
    if not (tw * (1 - tol) <= width <= tw * (1 + tol)) or not (th * (1 - tol) <= height <= th * (1 + tol)):
        return (
            f"{kind.value} must be about {tw}x{th}px (±{spec['tolerance_pct']}%). "
            f"Got {width}x{height}px. Please re-export at the correct size."
        )
    rw, rh = spec["ratio"]
    actual_ratio = width / height
    expected_ratio = rw / rh
    if abs(actual_ratio - expected_ratio) > 0.03:
        return f"{kind.value} must be {rw}:{rh}. Got a {width}:{height} image — that's the wrong aspect ratio."
    return None
...
async def upload_artwork(
    ...
    if len(raw) > MAX_ARTWORK_BYTES:
        raise HTTPException(
            422,
            f"File is {len(raw) // 1024}KB — the limit is 200KB. "
            f"Please compress the image before re-uploading.",
        )
    ...
    try:
        ...
        raise HTTPException(422, "That file isn't a readable image. Please upload a JPG or PNG.")

    err = _validate_dimensions(kind, width, height)
    if err:
        raise HTTPException(422, err)

    key = f"artwork/{kind.value}/{uuid.uuid4()}.{(file.filename or 'jpg').split('.')[-1]}"
    get_storage().put(key, raw)

    artwork = Artwork(
        kind=kind, storage_key=key, width=width, height=height,
        size_bytes=len(raw), show_id=show_id, episode_id=episode_id,
    )
    db.add(artwork)
    db.commit()
    db.refresh(artwork)
...
def validation_report(db: Session = Depends(get_db), _user: User = Depends(require_editor)):
...
def publish_catalog(db: Session = Depends(get_db), user: User = Depends(require_admin)):
    return run_publish(db, triggered_by=user.username)


@router.get("/catalog/publish-runs", response_model=list[PublishRunOut])
def publish_run_history(db: Session = Depends(get_db), _user: User = Depends(require_editor)):
